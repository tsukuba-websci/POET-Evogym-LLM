...
def make_gif(
    ...
    if track:
        ...
        view_size = (
            grid_size[0] + padding[0][0] + padding[0][1],
            grid_size[1] + padding[1][0] + padding[1][1],
        )
        resolution = (view_size[0] * resolution_scale, view_size[1] * resolution_scale)
        camera_position = (
            (grid_size[0] - padding[0][0] + padding[0][1]) / 2,
            (grid_size[1] - padding[1][0] + padding[1][1]) / 2,
        )
...
def make_jpg(
    ...
    resolution_scale=32,
    start_timestep=0,
    timestep_interval=80,
    distance_interval=0.8,
    blur=0,
    blur_temperature=0.6,
    display_timestep=False,
    ...
    while not done:
        ...
        if interval == "timestep":
            if (
                time >= start_timestep
                and (time - start_timestep) % timestep_interval == 0
            ):
                draw = True
            ...
            if (
                time >= start_timestep
                and np.linalg.norm(position - prev_position) > distance_interval
            ):
                draw = True
                mix_value = len(images) + 3
    ...
    if blur > 0:
        for draw_time in draw_times:
            for diff, m in enumerate(
                np.linspace(0.2, 1.0, blur)[: min(blur, draw_time)]
            ):
    ...
    image = sum([image * alpha for image, alpha in images]) / sum(
        [alpha for _, alpha in images]
    )
    image = image.astype("uint8")

    fig, ax = plt.subplots(
        figsize=(view_size[0] / 3, view_size[1] / 3), dpi=4 * resolution_scale
    )
    ax.imshow(image)
    ...
    if display_timestep:
        for draw_time in draw_times:
            ...
            y = (
                padding[1][1] + grid_size[1] - position[1] * 10 - 3.2
            ) * resolution_scale
...
class EvogymControllerDrawerPPO:
    def __init__(
        self, save_path, env_id, robot, overwrite=True, save_type="gif", **draw_kwargs
    ):
    ...
    def draw(self, trial, terrain_file, ppo_file, directory=""):
        ...
        env = make_vec_envs(
            self.env_id, env_kwargs, 0, 1, allow_early_resets=False, vecnormalize=True
        )
