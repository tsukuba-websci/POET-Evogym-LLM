...
def main():
    ...
    niches = [name for name in os.listdir(expt_niche) if os.path.isdir(os.path.join(expt_niche, name))]
    diff_niche = sorted(niches)
    
    dir_path = './out/evogym_poet/default/niche'
    dire = [name for name in os.listdir(dir_path) if os.path.isdir(os.path.join(dir_path, name))]
