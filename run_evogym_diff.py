...
def main():
    ...
    for i in range(len(directories)):
        model_path = os.path.join(dir_path, directories[i], "core", "best.pt")
        model = torch.load(model_path)
        for j in range(len(diff_niche)):
            niche_path = os.path.join(expt_path, "niche", niche_path[j])
            assert os.path.exists(niche_path), f"no niche key {niche_path[j]}"
            result_path = os.path.join(niche_path, "diff_ppo_result_{i}")
