...
from libs.experiment_utils import load_experiment
...
def main(expt_name):
    ...
    max_reward = expt_args["width"] / 10 + (
        10 if expt_args["task"] == "Parkour-v1" else 0
    )
    ...
    for niche_align, niche_key in enumerate(order):
        ...
        history = pd.read_csv(
            history_file,
            dtype={"step": "Int64", "reward": "Float64", "transferred_from": "Int64"},
        )
        ...
        rewards = [
            rewards[i * interval : (i + 1) * interval].max()
            for i in range(convolved_length)
        ]
        colorbar = ax.scatter(
            iterations,
            [niche_align] * len(iterations),
            vmin=0,
            vmax=max_reward,
            c=rewards,
            cmap="plasma",
        ...
        transfer_froms = [
            transfer_froms[k * interval : (k + 1) * interval].max()
            for k in range(convolved_length)
        ]
        ...
        while iter_i < convolved_length:
            ...
            valid = 0
            while iter_i + valid < convolved_length and transfer_froms[
                iter_i + valid
            ] in [-1, from_key]:
                valid += 1
            ...
            if (
                from_key != -1
                and rewards_cummax[iter_i + valid - 1] > reward_past_max
                and not (iter_i == 0 and from_key == parent_key)
            ):
                ...
                if from_align > niche_align:
                    ...
                    connectionstyle = f"arc3,rad=-{rad}"
                improve = (
                    rewards_cummax[iter_i + valid - 1] - reward_past_max
                ) / max_reward
                ...
                arrows.append(
                    patches.FancyArrowPatch(
                        ...
                        alpha=alpha,
                        lw=lw,
                        color=niches_dict[from_key]["color"],
                        **arrow_kwargs,
                    )
                )
