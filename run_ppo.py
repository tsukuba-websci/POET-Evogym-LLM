...
def run_ppo(
    ...
    eval_envs = make_vec_envs(
        env_id, robot, config.seed, config.eval_processes, gamma=None, vecnormalize=True
    )
    ...
    if save_iter:
        interval = time.time()
        torch.save(
            [policy.state_dict(), train_envs.obs_rms], os.path.join(save_file, "0.pt")
        )

        history_header = ["iteration", "reward"]
        items = {"iteration": 0, "reward": 0}
    ...
    for iter in range(train_iters):
        ...
        if (iter + 1) % eval_interval == 0:
            ...
            reward = evaluate(
                policy,
                eval_envs,
                num_eval=config.eval_processes,
            ...
            if reward > max_reward:
                ...
                if not save_iter:
                    torch.save(
                        [policy.state_dict(), train_envs.obs_rms], save_file + ".pt"
                    )
            ...
            if save_iter:
                ...
                print(
                    f"iteration: {iter+1:=5}  elapsed times: {now-interval:.3f}  reward: {reward:6.3f}  log_std: {log_std:.5f}"
                )
                interval = now

                torch.save(
                    [policy.state_dict(), train_envs.obs_rms],
                    os.path.join(save_file, f"{iter+1}.pt"),
                )

                items = {"iteration": iter + 1, "reward": reward}
