...
class BaseCPPNDecoder:
    ...
    def decode(self, genome, config):
        nodes = create_cppn(
            genome, config, leaf_names=self.inputs.keys(), node_names=self.output_keys
        )
...
class BaseHyperDecoder:
    ...
    def set_attr(self, substrate, connections, downstream_nodes):
        ...
        self.edge_labels, self.edge_inputs = substrate.get_connection_inputs(
            connections
        )
    ...
    def decode(self, genome, config):
        ...
        nodes = create_cppn(
            genome, config, leaf_names=self.edge_inputs.keys(), node_names=["value"]
        )
