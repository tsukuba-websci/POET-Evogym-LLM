...
class EvogymTerrainDecoder(neat_cppn.BaseCPPNDecoder):
    ...
    def decode(self, genome, config, terrain_param):
        ...
        for platform in platforms.values():
            ...
            platform["indices"] = [
                x + (y + start_height) * grid_width for x, y in points
            ]
            platform["neighbors"] = {
...
class TerrainParams:
    ...
    def reproduce(self, key):
        ...
        max_rigid_width = max(
            1, min(10, self.max_rigid_width + np.random.normal(0.00, 0.8))
        )
        max_soft_width = max(
            1, min(8, self.max_soft_width + np.random.normal(0.15, 0.8))
        )
        max_empty_width = max(
            1, min(7, self.max_empty_width + np.random.normal(0.15, 0.8))
        )
...
class EnvrionmentEvogymConfig:
    def __init__(
        self, robot, neat_config, env_id="Parkour-v0", max_width=80, first_platform=10
    ):
