...
class Niche:
    ...
    def start_evaluate(
        self, pool, env_config, opt_config, imigrant_cores=None, invasion=False
    ):
        assert len(self.processes) == 0
        ...
        if imigrant_cores is None:
            ...
            for imigrant_key, imigrant_core in imigrant_cores.items():
                if not invasion and imigrant_key in self.transferred_rewards:
                    ...
                    func, kwargs = self.optimizer.get_evaluate(
                        opt_config, core=imigrant_core
                    )
    ...
    def end_evaluate(
        ...
        if invasion:
            ...
            max_imigrant_key, max_imigrant_reward = max(
                rewards.items(), key=lambda z: z[1]
            )
            if max_imigrant_reward > self.reward:
                max_imigrant_core = imigrant_cores[max_imigrant_key]
                self.optimizer.set_core(
                    max_imigrant_core, opt_config, reset=reset_optimizer
                )
    ...
    def get_accepted_keys(self, keys):
        ...
        accepted_keys = [
            key
            for key in keys
            if key != self.key and self.transferred_rewards[key] > base_reward
        ]
