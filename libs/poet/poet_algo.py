...
class POET:
    ...
    def transfer(self, reciever_niches, child=False):
        ...
        imigrant_niche_cores = {
            niche_key: niche.get_optimizer_core()
        ...
        for reciever_key, reciever_niche in reciever_niches.items():
            accepted_keys = reciever_niche.get_accepted_keys(
                imigrant_niche_cores.keys()
            )
            if len(accepted_keys) == 0:
                continue

            accepted_cores = {
                accepted_key: imigrant_niche_cores[accepted_key]
    ...
    def get_novelty_score(self, niche, archives):
        ...
        for _, archive_niche in archives.items():
            ...
            archive_transferred_rewards = np.clip(
                archive_transferred_rewards,
                self.clip_reward_lower,
                self.clip_reward_upper,
            )
    ...
    def get_child_niches(self, reproduction_num):
        ...
        niche_cores = {
            key: niche.get_optimizer_core() for key, niche in self.niches.items()
        }
        ...
        parent_niches = [
            ...
            if niche.reward > self.repro_threshold
        ]
        ...
        all_niche_cores = {
            key: niche.get_optimizer_core() for key, niche in all_niches.items()
        }
        ...
        child_niches_pass = {
            key: niche
            for key, niche in child_niche_candidates.items()
            if self.pass_mc(niche)
        }
    ...
    def start_iteration(self):
        print(
            f"********************  ITERATION {self.iteration+1: =6}   ********************"
        )
    ...
    def end_iteration(self):
        save_core = (
            self.save_core_interval > 0
            and (self.iteration + 1) % self.save_core_interval == 0
        )
        for key, niche in self.niches.items():
