...
register(
    id="Parkour-v0", entry_point="custom_envs.parkour:Parkour", max_episode_steps=500
)
...
class Parkour(BenchmarkBase):
    ...
    def __init__(self, body, connections, terrain):
        ...
        self.world.add_from_array(
            "robot", body, 1, terrain["start_height"] + 1, connections=connections
        )
        ...
        self.action_space = spaces.Box(
            low=0.6, high=1.6, shape=(num_actuators,), dtype=np.float
        )
        self.observation_space = spaces.Box(
            low=-100.0,
            high=100.0,
            shape=(3 + num_robot_points + (2 * self.sight_dist + 1),),
            dtype=np.float,
        )
