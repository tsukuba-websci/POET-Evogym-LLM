...
def make_env(env_id, env_kwargs, seed, allow_early_resets=True):
    def _init():
        ...
        env.action_space = gym.spaces.Box(
            low=-1.0, high=1.0, shape=env.action_space.shape, dtype=np.float
        )
        env.seed(seed)
...
def make_vec_envs(
    ...
    envs = [
        make_env(env_id, env_kwargs, seed + i, allow_early_resets=allow_early_resets)
...
def load_robot(ROOT_DIR, robot_name, task=None):
    ...
    robot = {"body": body, "connections": connections}
    return robot
